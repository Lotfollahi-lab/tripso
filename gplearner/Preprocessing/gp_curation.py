...
from ..Utils.utils import (
    ...
    remove_leading_numbers_and_underscore,
)
...
def make_gpdb(
    ...
    # Save
    gpdb.to_csv(os.path.join(output_path, f'{name_tag}gpdb.csv'), index=False)

    # Visualize
    intersection_heatmap(
        gpdb, save_to=os.path.join(output_path, f'{name_tag}gpdb_heatmap.png')
    )

    # Export similarity matrix based on overlap
    make_similarity_matrix(
        gpdb, save_to=os.path.join(output_path, f'{name_tag}gp_similarity_matrix.npy')
    )
...
def print_results(df, intersection_matrix):
    for gp in df.columns:
        print(f'{gp} : {len(df[gp].dropna())} genes')
        print(
            'Maximum overlap:',
        )
