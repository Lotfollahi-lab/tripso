...
'''
LaminDB Mapped Collection class
https://github.com/laminlabs/lamindb/blob/main/lamindb/core/_mapped_collection.py#L330
accessed 30.09.2024
'''
...
# Other lamin imports
# mainly from
# https://github.com/laminlabs/lamindb/blob/main/
# lamindb/core/storage/_anndata_accessor.py
# accessed 30.09.2024
# ==============================================================================
    ...
    if encoding_type == '':
        if is_dataset:
            ...
            try:
                ...
                raise ValueError(
                    'Encountered an exception while attempting'
                    f'to subset a sparse dataset by indices.\n{e}'
                )
        if result is None:
            raise ValueError(
...
# ==============================================================================
# From upath
# https://github.com/laminlabs/lamindb-setup/blob/main/
...
class MappedCollection:
    ...
    the `AnnData` objects in `path_list`.
    The dictionary has keys for `layers_keys` (`.X` is in `"X"`),
    `obs_keys`, `obsm_keys` (under `f"obsm_{key}"`) and also `"_store_idx"`
    ...
    Args:
        path_list:
            A list of paths to `AnnData` objects
            stored in `.h5ad` or `.zarr` formats.
        layers_keys:
            Keys from the ``.layers`` slot.
            ``layers_keys=None`` or ``"X"`` in the list retrieves ``.X``.
        obsm_keys: Keys from the ``.obsm`` slots.
        obs_keys: Keys from the ``.obs`` slots.
        obs_filter:
            Select only observations with these values for the given obs column.
            Should be a tuple with an obs column name as the first element
            and filtering values (a string or a tuple of strings)
        ...
        unknown_label: Encode this label to -1.
            Can be a dictionary with keys from ``obs_keys``
            if ``encode_labels=True``
            or from ``encode_labels`` if it is a list.
        cache_categories: Enable caching categories of ``obs_keys``
            for faster access.
    ...
    def __init__(
        ...
        if self.filtered and len(obs_filter) != 2:  # mypy: ignore[arg-type]
            raise ValueError(
        ...
        if encode_labels and isinstance(unknown_label, dict):
            if not all(
                unkey in encode_labels for unkey in unknown_label
            ):  # type: ignore
                raise ValueError(
                    'All keys of `unknown_label` should be'
                    'in `encode_labels` and `obs_keys`.'
                )
    ...
    def _get_data_idx(
        ...
        if isinstance(lazy_data, ArrayTypes):  # type: ignore
            ...
            if join_vars == 'outer':
                ...
                lazy_data_idx = np.zeros(
                    lazy_data.attrs['shape'][1], dtype=dtype
                )  # type: ignore
    ...
    def get_label_weights(
        ...
        weights for each obs label accoding to the formula
        `1 / num of this label in the data`.
        If `scaler` is provided,
        then `scaler / (scaler + num of this label in the data)`.

        Args:
            obs_keys:
                A key in the ``.obs`` slots or a list of keys.
                If a list is provided,
                the labels from the obs keys will be concatenated
                with ``"__"`` delimeter
            scaler:
                Use this number to scale the provided weights.
            return_categories:
                If `False`, returns weights for each observation,
                can be directly passed to a sampler.
                If `True`, returns a dictionary with
                unique categories for labels
                (concatenated if `obs_keys` is a list)
                and their weights.
        """
