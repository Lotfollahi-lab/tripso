...
def get_token_attributions(b, ig, device, obs_key='cell_type', obs_value='cell_type_1'):
    '''
    How much does each a GP depend on each gene?
    For a given batch, get the attributions for the tokens
    NB batch size = 1
    but we check the metadata first
    '''
    if obs_value in b[1][obs_key]:
        emb = b[0].to(device)
        edict = {
        ...
        attribution_df = pd.DataFrame(
            {
                ...
                'attribution_score': attributions.detach().cpu().numpy(),
            }
        )
        ...
        attribution_df['gene_rank'] = attribution_df['token'].apply(
            ...
            if x in token_labels_list
            else len(token_labels_list) + 1
        )
        ...
        for token in attribution_df['token']:
            ...
            tracker[f'{token}_attribution_score'] = tmp['attribution_score'].values[0]
...
def get_gp_attributions(
    b, gp_inputs, ig, device, obs_key='cell_type', obs_value='cell_type_1'
):
    '''
    How much does each cell token depend on each GP?
    '''
    if b[1][obs_key] == [obs_value]:
        ...
        attribution_df = pd.DataFrame(
            {
                'GP': gp_inputs,
                'attribution_score': summarize_attributions(attributions)
                .detach()
                .cpu()
                .numpy(),
        ...
        for g in attribution_df['GP']:
            ...
            tracker[f'{g}_attribution_score'] = tmp['attribution_score'].values[0]
            # number of genes per cell already divided by total GP length when defined
            tracker[f'{g}_prop_genes'] = tmp['num_genes_per_cell'].values[0]
