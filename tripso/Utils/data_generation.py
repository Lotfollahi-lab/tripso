...
Code adapted from https://github.com/mateoespinosa/tabcbm

Mainly taken from

https://github.com/mateoespinosa/tabcbm/blob/main/data/scsim.py
...
class scsim:
    def __init__(
        ...
        min_act_prog_usage=0.2,
        max_act_prog_usage=0.8,
        gp_overlap=0,
    ):
        if ((diffexpprob is None) and (diff_exp_prob_genes is None)) or (
            (diff_exp_prob_genes is not None) and (diffexpprob is not None)
        ):
            raise ValueError(
                'Either diff_exp_prob_genes or diffexpprob need to be provided'
            )

        if diff_exp_prob_genes is not None:
            if len(diff_exp_prob_genes) != n_cell_types:
                raise ValueError(
                    f'Expected as many lists in diff_exp_prob_genes'
                    '(found {len(diff_exp_prob_genes)}) '
            ...
            diffexpprob = [
                [diffexpprob for i in range(ngenes)] for _ in range(n_cell_types)
            ]

        if (act_prog_gene_sizes is not None) and (act_prog_genes is not None):
            raise ValueError(
                'Expected at most one of act_prog_gene_sizes'
                'or act_prog_genes to be provided.'
            )
        ...
        self.gp_overlap = gp_overlap

        # How many randomly selected genes will be involved in each activity program
        self.act_prog_gene_sizes = act_prog_gene_sizes
        if (self.act_prog_gene_sizes is not None) and (
            not isinstance(self.act_prog_gene_sizes, (list, np.ndarray))
        ):
            ...
            if act_prog_genes is not None:
                ...
                for i, n_genes_in_act_prog in enumerate(self.act_prog_gene_sizes):
                    self.activity_program_genes[i] = np.sort(
                        np.random.choice(
                            self.ngenes, n_genes_in_act_prog, replace=False
                        )
                    )
        ...
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.act_prog_down_prob, (list, np.ndarray)):
                self.act_prog_down_prob = [
                    self.act_prog_down_prob for _ in self.act_prog_gene_sizes
                ]
            elif len(self.act_prog_down_prob) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the probability of downregulating an activity'
                    'program to be a scalar or an iterable with '
                    f'as many elements as provided activity programs.'
                    'Instead we obtained {self.act_prog_down_prob} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )

        self.act_prog_de_loc = act_prog_de_loc
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.act_prog_de_loc, (list, np.ndarray)):
                self.act_prog_de_loc = [
                    self.act_prog_de_loc for _ in self.act_prog_gene_sizes
                ]
            elif len(self.act_prog_de_loc) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the mean DE parameter an activity program'
                    'to be a scalar or an iterable with'
                    f'as many elements as provided activity programs.'
                    'Instead we obtained {self.act_prog_de_loc} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )
        ...
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.act_prog_de_scale, (list, np.ndarray)):
                self.act_prog_de_scale = [
                    self.act_prog_de_scale for _ in self.act_prog_gene_sizes
                ]
            elif len(self.act_prog_de_scale) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the scale DE parameter an activity program to '
                    'be a scalar or an iterable with '
                    f'as many elements as provided activity programs.'
                    'Instead we obtained {self.act_prog_de_scale} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )

        # Decide which cell types can be involved in each activity program.
        # If not given then we assume that
        # activity programs can be distributed across all cell types
        self.act_prog_cell_types = act_prog_cell_types
        if (self.act_prog_gene_sizes is not None) and (
            self.act_prog_cell_types is None
        ):
        ...
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.act_prog_cell_types[0], (list, np.ndarray)):
                self.act_prog_cell_types = [
                    np.array(self.act_prog_cell_types) for _ in self.act_prog_gene_sizes
                ]
            elif len(self.act_prog_cell_types) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the set of cell types involved with each'
                    'activity prgram to be a single list of integers or a '
                    f'list with as many lists as provided activity programs.'
                    'Instead we obtained {self.act_prog_cell_types} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )
        ...
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.act_prog_cell_frac, (list, np.ndarray)):
                self.act_prog_cell_frac = [
                    self.act_prog_cell_frac for _ in self.act_prog_gene_sizes
                ]
            elif len(self.act_prog_cell_frac) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the number of activity program cell frequencies to'
                    'be a scalar or an iterable with '
                    f'as many elements as provided activity programs.'
                    'Instead we obtained {self.act_prog_cell_frac} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )
        ...
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.min_act_prog_usage, (list, np.ndarray)):
                self.min_act_prog_usage = [
                    self.min_act_prog_usage for _ in self.act_prog_gene_sizes
                ]
            elif len(self.min_act_prog_usage) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the min activity program usuage parameter to be'
                    'a scalar or an iterable with '
                    f'as many elements as provided activity programs.'
                    'Instead we obtained {self.min_act_prog_usage} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )
        ...
        if self.act_prog_gene_sizes is not None:
            if not isinstance(self.max_act_prog_usage, (list, np.ndarray)):
                self.max_act_prog_usage = [
                    self.max_act_prog_usage for _ in self.act_prog_gene_sizes
                ]
            elif len(self.max_act_prog_usage) != len(self.act_prog_gene_sizes):
                raise ValueError(
                    'Expected the max activity program usuage parameter to be a'
                    'scalar or an iterable with '
                    f'as many elements as provided activity programs.'
                    'Instead we obtained {self.max_act_prog_usage} '
                    f'while we were also given {len(self.act_prog_gene_sizes)}'
                    'activity programs to simulate.'
                )
        ...
        elif (len(cell_type_probs) == self.n_cell_types) and (
            np.abs(np.sum(cell_type_probs) - 1) < (1e-6)
        ):
    ...
    def simulate(self):
        ...
        if (self.act_prog_gene_sizes is not None) and (
            len(self.act_prog_gene_sizes) > 0
        ):
    ...
    def get_cell_gene_means(self):
        ...
        cell_type_gene_mean = self.geneparams.loc[
            ...
            [
                ...
                if ('_gene_mean' in x) and ('cell_type' in x)
            ],
        ].T.astype(float)
        cell_type_gene_mean = cell_type_gene_mean.div(
            cell_type_gene_mean.sum(axis=1), axis=0
        )
        ...
        if (self.act_prog_gene_sizes is None) or (len(self.act_prog_gene_sizes) == 0):
            ...
            no_act_prog_cells = (
                np.sum(
                    np.array(
                        list(map(list, self.cellparams['has_act_program'].values))
                    ),
                    axis=-1,
                )
                == 0
            ...
            act_prog_usage = self.cellparams.loc[
                act_prog_cell_mean.index, ['act_program_usage']
            ]
            act_prog_usage.columns = ['act_prog_gene_mean']
            act_prog_cell_mean += act_prog_usage.dot(act_prog_mean.T)
            # TODO (verify this): if multiple identity programs use the same gene,
            # then its resulting mean will be the
            ...
            cell_gene_mean = pd.concat(
                [no_act_prog_cell_mean, act_prog_cell_mean],
                axis=0,
                verify_integrity=True,
            )
            # Sort them so that they are in the same order as before
            cell_gene_mean.sort_index(
                inplace=True, key=lambda x: [int(y[len('Cell') :]) for y in x]
            )
    ...
    def simulate_activity_programs(self):
        ...
        for act_prog_idx, n_genes_in_act_prog in enumerate(self.act_prog_gene_sizes):
            ...
            for cell_type in self.act_prog_cell_types[act_prog_idx]:
                # For each cell type, let's go ahead and simulate
                # the activity of this program if it is
                # relevant for this activity program at at all
                group_cells = self.cellparams.index[
                    self.cellparams['cell_type'] == cell_type
                ]

                # Determine which cells in this cell type group will
                # have this specific activation program in it
                # We do this using a random Bernoulli distribution
                # using a given expected fraction of cell types
                ...
                for vec in self.cellparams.loc[
                    group_cells[has_prog], 'has_act_program'
                ]:
                    vec[act_prog_idx] = 1
                ...
                for i, vec in enumerate(
                    self.cellparams.loc[group_cells[has_prog], 'act_program_usage']
                ):
    ...
    def sim_cell_type_DE(self):
        ...
        cell-type'''
        # Find out which genes are used for activity programs
        # already to avoid differientating those
        if (self.act_prog_gene_sizes is not None) and np.sum(
            self.act_prog_gene_sizes
        ) > 0:
            act_prog_genes = (
                np.sum(
        ...
        for i, cell_type in enumerate(self.cell_types):
            ...
            # for even cell types,
            # get a random sample of genes to act as GP
            ...
            if i % 2 == 0:
                for gene_idx in range(self.ngenes):
                    isDE[gene_idx] = np.random.choice(
                        [True, False],
                        p=[
                            self.diffexpprob[cell_type - 1][gene_idx],
                            1 - self.diffexpprob[cell_type - 1][gene_idx],
                        ],
                    )

                prev_genes_idx = np.where(isDE)[0]

            else:
                # for odd cell types,
                # reuse self.gp_overlap % genes from the previous cell type

                if self.gp_overlap > 0:
                    num_overlap_genes = int(self.gp_overlap * len(prev_genes_idx))
                    overlap_genes = np.random.choice(
                        prev_genes_idx, size=num_overlap_genes, replace=False
                    )

                    # Add (1 - self.gp_overlap) random indices
                    #  to ensure the same total number
                    random_genes = np.random.choice(
                        np.setdiff1d(range(self.ngenes), overlap_genes),
                        size=len(prev_genes_idx) - num_overlap_genes,
                        replace=False,
                    )

                    # Combine the overlap and random indices
                    selected_genes = np.concatenate((overlap_genes, random_genes))

                    # Populate isDE based on the selected genes
                    isDE[selected_genes] = True
            ...
            # Program genes shouldn't be differentially expressed between cell types

            isDE[act_prog_genes] = False
            DEratio = np.random.lognormal(
...
def generate_synthetic_sc_dataset(
    ...
    simulator = scsim(
        ...
        ndoublets=ndoublets,
        # based on HECA epithelial cells
        libloc=9.64,
        libscale=0.78,
        mean_rate=1.10,
        mean_shape=0.41,
        ...
        act_prog_down_prob=0.0,
        act_prog_de_loc=1.0,
        act_prog_de_scale=1,
        act_prog_cell_frac=0.3,
        act_prog_cell_types=act_prog_cell_types
        or [list(range(1, num_cell_types + 1, 1)) for _ in act_prog_gene_sizes],
        min_act_prog_usage=0.1,
        max_act_prog_usage=0.7,
        seed=seed,
        gp_overlap=gp_overlap,
    )
    simulator.simulate()
    ...
    adata.obs['label'] = [
        0 if np.sum(x) == 0 else 1 for x in adata.obs['has_act_program']
    ]
    adata.obs['label_str'] = [
        f'class_{0 if np.sum(x) == 0 else 1}' for x in adata.obs['has_act_program']
    ]
    adata.obs['activity_program_str'] = [str(x) for x in adata.obs['has_act_program']]

    sc.pp.filter_cells(adata, min_genes=min_genes)
    sc.pp.filter_cells(adata, min_counts=200)
    kept_gene_mask, num_per_gene = sc.pp.filter_genes(
    ...
    for cell_type in range(1, simulator.n_cell_types + 1):
        for activity_program_combo in itertools.product(
            *[[0, 1] for _ in simulator.act_prog_gene_sizes]
        ):
    ...
    adata.obs['combination_label'] = [
        combination_label_map[
            (adata.obs.cell_type[i], tuple(adata.obs.has_act_program[i]))
        ]
        for i in range(adata.shape[0])
    ]
    adata.obs['combination_label_str'] = [
        f'comb_label_{x}' for x in adata.obs['combination_label']
    ]
    ...
    # Plot the UMAP with some cannonical marker genes
    # to see that the apparent clustering makes sense
    adata.obs['cell_type_str_viz'] = [
        f'Identity GEP {i}' for i in adata.obs['cell_type']
    ]
    if plot:
        sc.pl.umap(
            adata,
    ...
    # Plot the UMAP with some cannonical marker genes
    # to see that the apparent clustering makes sense
    adata.obs['activity_program_str_viz'] = [
        'True' if x[0] else 'False' for x in adata.obs['has_act_program']
    ]
    if plot:
        sc.pl.umap(
            adata,
    ...
    c_train = np.concatenate(
        [
            np.eye(num_cell_types)[adata.obs['cell_type'].to_numpy() - 1],
            np.array([x for x in adata.obs['has_act_program'].to_numpy()]),
        ],
        axis=-1,
    )
    ...
    for prog_idx in range(num_cell_types):
        ground_truth_concept_masks.append(
            np.array(
                [
                    1
                    if simulator.geneparams[f'cell_type_{prog_idx + 1}_gene_selection'][
                        j
                    ]
                    else 0
                    for j in range(simulator.ngenes)
                    if kept_gene_mask[j]
                ]
            )
        )
    ...
    for prog_idx in range(len(simulator.activity_program_genes)):
        ground_truth_concept_masks.append(
            np.array(
                [
                    1 if j in simulator.activity_program_genes[prog_idx] else 0
                    for j in range(simulator.ngenes)
                    if kept_gene_mask[j]
                ]
            )
    ...
    if dataset_dir is not None:
        ...
        joblib.dump(adata, os.path.join(dataset_dir, f'adata_seed_{seed}.joblib'))
    return (
        X_train,
        X_test,
        y_train,
