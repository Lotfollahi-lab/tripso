...
Directly copied from https://huggingface.co/ctheodoris/Geneformer/blob/main/geneformer/tokenizer.py
This version accessed January 2026

**Input data:**

| *Required format:* raw counts scRNAseq data without feature selection as .h5ad or .zarr file.
...
| Input data is a directory with .h5ad or .zarr files containing raw counts from single cell RNAseq data, including all genes detected in the transcriptome without feature selection. The input file type is specified by the argument file_format in the tokenize_data function.

| Genes should be labeled with Ensembl IDs ("ensembl_id" in `data.var`), which provide a unique identifer for conversion to tokens. Other forms of gene annotations (e.g. gene names) can be converted to Ensembl IDs via Ensembl Biomart. Cells should be labeled with the total read count in the cell (`n_counts` in `data.obs`) to be used for normalization.

| No cell metadata is required, but custom cell attributes may be passed onto the tokenized dataset by providing a dictionary of custom attributes to be added, which is formatted as input_obs_attr_name : desired_dataset_col_attr_name. For example, if the original dataset has `obs` attributes "cell_type" and "organ_major" and one would like to retain these attributes as labels in the tokenized dataset with the new names "cell_type" and "organ", respectively, the following custom attribute dictionary should be provided: {"cell_type": "cell_type", "organ_major": "organ"}.

| Additionally, if the original dataset contains a cell `obs` attribute called "filter_pass", this column will be used as a binary indicator of whether to include these cells in the tokenized data. All cells with "1" in this attribute will be tokenized, whereas the others will be excluded. One may use this column to indicate QC filtering or other criteria for selection for inclusion in the final tokenized dataset.

| If one's data is in other formats besides .h5ad or .zarr, one can use the relevant tools (such as AnnData tools) to convert the file to one of these formats prior to running the transcriptome tokenizer.
...
def sum_ensembl_ids(
    ...
    if file_format not in ['h5ad', 'zarr']:
        raise ValueError(
            f"Unsupported file format '{file_format}'. Use 'h5ad' or 'zarr'."
        )
...
class TranscriptomeTokenizer:
    def __init__(
        ...
        **Parameters:**
        ...
        custom_attr_name_dict : None, dict
            ...
            | Keys are the names of the attributes in the input data `obs`.
        ...
        """
        # dictionary of custom attributes {output dataset column name: input obs column name}
        ...
        # protein-coding and miRNA gene list dictionary for selecting genes for tokenization
    ...
    def tokenize_data(
        ...
        """
        Tokenize files in data_directory and save as tokenized .dataset in output_directory.

        **Parameters:**

        data_directory : Path
            | Path to directory containing input anndata files
        output_directory : Path
            | Path to directory where tokenized data will be saved as .dataset
        output_prefix : str
            | Prefix for output .dataset
        file_format : str
            | Format of input files. Can be "h5ad", or "zarr".
        input_identifier : str
            | Substring identifier for input .h5ad, or .zarr, only matches are tokenized
    ...
    def tokenize_files(
        ...
        for file_path in data_directory.glob(file_match):
            ...
            (
                file_tokenized_cells,
                file_cell_metadata,
                file_tokenized_counts,
            ) = self.tokenize_anndata(file_path, file_format=file_format)
    ...
    def tokenize_anndata(self, adata_file_path, target_sum=10_000, file_format='h5ad'):
        ...
        empty_cell_indices = [
            i for i, cell in enumerate(tokenized_cells) if cell.size == 0
        ]
        ...
        def format_cell_features(example):
            ...
            if self.special_token:
                ...
                if self.keep_counts:
                    ...
                    example['counts'] = np.insert(example['counts'], 0, 0.0)
