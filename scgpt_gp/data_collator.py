...
class DataCollator:
	...
	######################### Added By Hesam for gplearner ###########################
        gene_idx_tensor = data_dict['gene']
        gene_idx_tensor = gene_idx_tensor[:, 1:].clone() # removing cls
        gene_idx_tensor[gene_idx_tensor == 60694] = 0 # TODO: hard coded <pad> vocab idx value
        data_dict["input_ids"] = gene_idx_tensor

        if examples[0]["adata"]:
            adata_batch = [d["adata"] for d in examples]
            counts = torch.stack([d["X"] for d in adata_batch])
            data_dict["counts"] = counts
            data_dict["size_factor"] = [d["size_factor"] for d in adata_batch]

        metadata_keys = []
        for m in list(examples[0].keys()):
           if m.startswith('metadata_'):
               metadata_keys.append(m)
        for m in metadata_keys:
            m_ = m.removeprefix('metadata_')
            if m.endswith('_id') and isinstance(examples[0][m], numbers.Number):
                data_dict[m_] = torch.tensor(
                    [d[m] for d in examples], dtype=torch.long
                )
            else:
                data_dict[m_] = [d[m] for d in examples]
        ##################################################################################
        return data_dict
